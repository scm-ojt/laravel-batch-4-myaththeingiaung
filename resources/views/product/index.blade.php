                    ...
                    <div class="d-flex justify-content-between">
                        @if(auth()->guard('admin')->user())
                            <form action="{{ route('admin.product.import') }}" class="mt-2 d-flex align-items-start justify-content-start" style="margin-left: 15px" method="post" enctype="multipart/form-data">
                                ...
                                <input id="file" type="file" class="form-control pt-1 @error('file') is-invalid @enderror" name="file" value="{{ old('file') }}" autocomplete="file">
                                @error('file')
                                ...
                                <button class="btn btn-warning" style="margin: 0 5px;">Import</button>
                      ...
                      <tbody>
                        ...
                        @else
                            ...
                            <td>
                                @if (auth()->guard('admin')->user())
                                    <div class="d-flex">
                                        <form class="" style="margin-right: 10px;" action="{{ route('admin.product.destroy',$product->id) }}"  method="post">
                                            @csrf
                                            @method('delete')
                                ...
                                @elseif(auth()->user()->id == $product->user?->id)
                                    <div class="d-flex">
                                        ...
                                        <div class="col2">
                                        <a class="btn btn-secondary d-block text-decoration-none text-white" href="{{ route('admin.product.edit',$product->id) }}"><i class="fa-solid fa-pen" style="margin-right: 10px"></i>Edit</a>
                                        </div>
                                    </div>
                                @endif
                            </td>
